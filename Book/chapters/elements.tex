...
\begin{block}{Return statement}
  ...
  \item Returning zero means success; anything else failure;
  \item This \indextermsub{return}{code} can be detected by the Operating System.
  \end{itemize}
...
\begin{block}{Initialization syntax}
  There are two ways of initializing a variable\index{variable!initialization}:
  \begin{lstlisting}
    int i = 5;
    int j{6};
  \end{lstlisting}
...
\end{block}
  ...
  Floating points numbers do not behave like mathematical
  numbers. 
...
and
  \begin{enumerate}
  \item Read a \n{float eps},
  \item Make a new variable that has the value \n{one+eps}. Print
    this.
  \item Make another variable that is the previous one mine
    \n{one}. Print the result again.
