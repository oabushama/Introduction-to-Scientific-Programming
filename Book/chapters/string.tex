...
\begin{exercise}
  ...
  decimal:
  %
  \answerwithoutput{qfcall}{string}{quasifix}
  %
...
\Level 1 {Conversation to/from string}

There are various mechanisms for converting between strings and numbers.
\begin{itemize}
\item The C legacy mechanisms \indextermtt{sprintf} and \indextermtt{itoa}.
\item  \indextermtt{to_string}
\item Above you saw the \indextermtt{stringstream}; section~\ref{sec:stringstream}.
\item The \indexterm{Boost} library has a \indextermsub{lexical}{cast}.
\end{itemize}

Additionally, in \emph{C++17}\index{C++!C++17} there is the
\indextermdef{charconv} header with \indextermtt{to_chars} and
\indextermtt{from_chars}. These are low level routines that do not
