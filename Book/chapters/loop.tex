...
for (int i=0; i<m; i++)
  for (int j=0; j<n; j++)
    ...
\end{verbatim}
The \n{i}-loop is called the \indextermsub{outer}{loop}; the
\n{j}-loop the \indextermsub{inner}{loop}.
...
\begin{exercise}
  ...
  Output one line for each \n{i} value. Food for thought: did you use a
...
%\ifIncludeAnswers\vfill\eject\fi
