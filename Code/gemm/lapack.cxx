...
public:
  Matrix(int m,int n,double v=0)
    : m(m),n(n),lda(m),
      data( vector<double>(m*n,v) ) { cout << "alloc" << endl; };
  Matrix(int m,int n,int lda,double *data)
    : m(m),n(n),lda(lda),
      data( span<double>(data,lda*n) ) { cout << "span" << endl; };
...
#define AT(i,j,m,n,lda) (i)+(j)*(lda)
  ...
  Matrix operator+( Matrix other ) const {
    if (other.m!=m or other.n!=n) {
      cout << "Operator+: incompatible dimensions" << endl;
      throw(1); }
    Matrix out(m,n);
    for (int j=0; j<n; j++)
      for (int i=0; i<m; i++)
  ...
  auto min() {
	...
	try {
	  ...
	  cout << "min out of range: " << i << "," << j << endl;
	  throw(1); }
#else
	v = matdata[ i+j*lda ];
#endif
  ...
  auto max() {
	...
	try {
	  ...
	  cout << "max out of range: " << i << "," << j << endl;
	  throw(1); }
#else
	v = matdata[ i+j*lda ];
#endif
  ...
  void MatMult( Matrix& other,Matrix& out ) {
    if (n!=other.m || m!=out.m || other.n!=out.n) {
      cout << "MatMult: incompatible dimensions" << endl;
      throw(1); }
...
int main() {
  ...
  cout << "\nMatMult:" << endl;
  {
    ...
    Matrix A(M,N,1.),B(M,N,2.),C(M,N);
    {
      ...
      cout << "product took: " << duration << " milliseconds" << endl;
      cout << "checking:" << endl;
      auto min = C.min(), max = C.max();
      cout << "C range: " << min << "--" << max << endl;
    }
    {
      auto start_time = high_resolution_clock::now();
      ...
      auto end_time = high_resolution_clock::now();
      auto duration = duration_cast<milliseconds>(end_time - start_time).count();
      cout << "recursive product took: " << duration << " milliseconds" << endl;
      cout << "checking:" << endl;
      auto min = C.min(), max = C.max();
      cout << "C range: " << min << "--" << max << endl;
    }
