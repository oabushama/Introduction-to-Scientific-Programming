...
tuple<int,int,int> string_find_suffix(string basestring,int baseloc,string read) {
  ...
  for ( ;
    ...
    ;
  //cout << "at " << genepointer << " match on " << basestring[genepointer] << endl ;
  if (suffixlength<readlength)
    cout << "at " << genepointer << " mismatch on " << basestring[genepointer] << endl ;
...
int find_mismatched_char_in_read(char badchar,string read,int readpointer) {
  ...
  for ( ; readpointer-badcharskip>=0 && read[readpointer-badcharskip]!=badchar;
	badcharskip++ )
    ;
  cout << "looking for " << badchar << " we skip back by " << badcharskip << endl ;
  return badcharskip;
}
...
public:
  ...
  tuple<int,int,int> find_suffix(int baseloc,string read) {
    return string_find_suffix(basestring,baseloc,read); }
  tuple<bool,int> matchread( string read,int strategy=3 ) {
    auto readlength = read.size();
    int baseloc=0;
    for ( ; baseloc<=basestring.size()-readlength; ) {
	...
	return {true,baseloc};
      ...
      auto badcharskip = find_mismatched_char_in_read(basestring[genepointer],read,readpointer);
      baseloc += badcharskip;
    }
    return {false,-1};
  };
...
auto ReadFastQ( string filename ) {
  ...
  while (input_file) {
    ...
    if (baseline.size()!=qualline.size()) {
      cout << "Base/quality length mismatch at base " << basecount << endl;
      break;
    }
...
int main() {
  ...
  // first some unit tests
  {
    ...
    {
      auto [match,loc] = test1.matchread("AT");
      ASSERT(match==true,"AT matched");
      ASSERT(loc==0,"AT matched at zero");
    }
    {
      auto [match,loc] = test1.matchread("ATTC");
      ASSERT(match==true,"ATTC matched");
      ASSERT(loc==0,"ATTC matched at zero");
    }

    genome test2("ATTC",true);
    {
      auto [match,loc] = test2.matchread("TT",1);
      ASSERT(loc==1,"TTC matched at one");
    }
    {
      auto [match,loc] = test2.matchread("TTC",1);
      ASSERT(loc==1,"TTC matched at one");
    }
    {
      auto [match,loc] = test2.matchread("TTC",2);
      ASSERT(loc==1,"TTC matched at one");
    }

    genome test3("GATCTACTTCA",true);
    { // find perfect match with badcharskip
      auto [match,loc] = test3.matchread("TTC",2);
      ASSERT(match==true,"TTC matched");
      ASSERT(loc==7,"TTC matched at 7");
    }
  ...
  cout << "Genome has length " << phix.size() << endl;
  ...
  { // using fake reads
    ...
    for ( auto r : reads ) {
      ...
      // if (match) 
      // 	cout << "Match: " << r << " @ " << loc << endl;
      // else
      // 	cout << "No match on " << r << endl;
      nmatch += match;
    }
    cout << "Number of matches: " << nmatch << " out of " << reads.size() << endl;
  }
